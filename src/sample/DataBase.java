...
public class DataBase {
    ...
    public void createWineObjects(){
        ...
        try {
            ResultSet count = this.st.executeQuery("SELECT COUNT(*) AS total FROM PRODUCTS WHERE PRODUCTTYPE = 'WINE'");
            while (count.next()) {
                rows = count.getInt("total");
            }

            for (int i = 0; i < rows; i++) {
                ResultSet rs = this.st.executeQuery("SELECT * FROM PRODUCTS WHERE PRODUCTTYPE= 'WINE' LIMIT " + index + "," + one);
